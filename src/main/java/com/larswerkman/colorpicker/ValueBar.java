...
public class ValueBar extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (!isInEditMode()) {
			shader = new LinearGradient(
					...
					(mBarLength + mBarPointerHaloRadius),
					mBarThickness,
					new int[] { Color.HSVToColor(0xFF, mHSV), Color.BLACK },
			...
			Color.colorToHSV(0xff81ff00, mHSV);
		}
		...
		if (!isInEditMode()) {
			mBarPointerPosition = Math .round((mBarLength - (mSatToPosFactor * hsvColor[2]))
							                    + mBarPointerHaloRadius);
		} else {
			mBarPointerPosition = mBarPointerHaloRadius;
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (x >= (mBarPointerHaloRadius) && x <= (mBarPointerHaloRadius + mBarLength)) {
				mBarPointerPosition = Math.round(x);
				mHSV[2] = calculateValue(mBarPointerPosition);
     ...
     * EDIT Andrei: It is HIGHLY encouraged to use this method.
	 * 
	 * @param color
	 */
	public void setColor(int color) {
        ...
        mBarPointerPosition = Math.round((mBarLength - (mSatToPosFactor * mHSV[2]))
                                           + mBarPointerHaloRadius);
		shader = new LinearGradient(mBarPointerHaloRadius, 0,
	...
	protected Parcelable onSaveInstanceState() {
		...
		state.putFloatArray(STATE_COLOR, mHSV);
