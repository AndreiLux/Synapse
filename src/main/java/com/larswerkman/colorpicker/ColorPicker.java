...
public class ColorPicker extends View {
	 ...
	 * is always called when the color is changed.
	 * 
	 * @author lars
	 * 
	 */
	...
	private int calculateColor(float angle) {
        ...
        return Color.HSVToColor(new float[] { degrees, mHSV[1], mHSV[2] });
	}
     ...
     *
     *            EDIT Andrei: Well fuck you. Now it's accurate.
     *                         Disregard above warning.
	 */
	public void setColor(int color) {
		mAngle = colorToAngle(color);
        mColor = color;
        Color.colorToHSV(mColor, mHSV);
        mPointerColor.setColor(Color.HSVToColor(new float[] { mHSV[0], 1f, 1f}));
	...
	protected void onRestoreInstanceState(Parcelable state) {
        ...
        Color.colorToHSV(currentColor, mHSV);
        mPointerColor.setColor(Color.HSVToColor(new float[] { mHSV[0], 1f, 1f}));
