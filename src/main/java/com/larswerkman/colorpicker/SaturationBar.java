...
public class SaturationBar extends View {
		...
		if(!isInEditMode()){
			shader = new LinearGradient(mBarPointerHaloRadius, 0,
					(mBarLength + mBarPointerHaloRadius), mBarThickness, new int[] {
							Color.WHITE,
							Color.HSVToColor(0xFF, mHSV) }, null,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (x >= (mBarPointerHaloRadius) && x <= (mBarPointerHaloRadius + mBarLength)) {
				...
				mHSV[1] = calculateSaturation(Math.round(x));
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsMovingPointer) {
				...
				if (x >= mBarPointerHaloRadius && x <= (mBarPointerHaloRadius + mBarLength)) {
					...
					mHSV[1] = calculateSaturation(mBarPointerPosition);
	...
	public void setColor(int color) {
        ...
        mBarPointerPosition = Math.round((mSatToPosFactor * mHSV[1])) + mBarPointerHaloRadius;
     ...
     */
	private float calculateSaturation(int x) {
